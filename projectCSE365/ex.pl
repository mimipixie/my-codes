?-nl,nl,nl,write('##Write lets_go_tour to start like lets_go_tour.  ##'), nl.
    lets_go_tour:-
    write('  ########A Beautiful Bangladesh is Waiting For You.  ########  '), nl,
    write(' If you want to explore press 1.    '), nl,  write('  If you want to exit press 2.    '), nl,nl,
    write('   1. Explore      '), nl,
    write('   2. Exit      '),nl,
    write('   Enter Your Choice:   '),nl, read(X), selection(X).
selection(X):- X==1,user; X==2,exit; X=\=1,X=\=2,write('    something went wrong try again...'),nl,nl,lets_go_tour.
exit:-
    write('          ####### sorry to see you go   #########'),nl,nl.
start:-
	create_file('username.txt',''),
	create_file('password.txt','').
user:-
	write('Are you new here? ( (Type y/n/exitwindow) '),nl, read(X),member_login(X).
member_login(exitwindow).
member_login(y):-
	write('Please enter user information :- '),nl,nl, write('Username : '),read(U),nl, write('Password : '),read(P),nl,
	read_from_userfile('username.txt',U,P,0,0,0,0,1).
member_login(n):-
	write('Please login with username and password :- '),nl,nl,write('Username : '),read(U),nl, write('Password : '),read(P),nl,
	read_from_userfile('username.txt',U,P,0,0,0,0,0).	
member_login(X):-
	write('Invalid input. Please type again.'),nl,nl,	user.
read_from_userfile(File,U,P,A,B,C,D,Z):-
    open(File, read, Str), read_file(Str,Lines,U,P,A,B,C,D,Z), close(Str).									
read_file(Stream,Lines,U,P,A,B,C,D,Z):-
    at_end_of_stream(Stream), notMatched(U,P,A,Q,C,D,Z).									
read_file(Stream,Lines,U,P,A,B,C,D,Z):-
    \+ at_end_of_stream(Stream),
(	( (A=:=0) ; (\+ (B = A))) ->
	(	readLine(Stream,Lines),Q is B+1,
		(	( (\+ (Lines = U))) ->
...
readLine(InStream,W):- 
    get_code(InStream,Char),     readC(Char,Chars,InStream),    atom_codes(W,Chars). 
readC(10,[],_):-  !. 
readC(32,[],_):-  !. 
readC(-1,[],_):-  !. 
readC(end_of_file,[],_):-  !. 
readC(Char,[Char|Chars],InStream):- 
	get_code(InStream,NextChar),  readC(NextChar,Chars,InStream).
write_to_file(File,Text):-
	open(File,append,Stream), 	write(Stream,Text),nl(Stream), 	close(Stream).
create_file(File,Text):-
	open(File,write,Stream),	write(Stream,Text),	close(Stream).
...
matched(U,P,A,B,C,D,1):-
	write('This username already exists. Please try another one.'),nl,nl,member_login(y).
notMatched(U,P,A,B,C,D,0):-
(	( (D =:= 0) , (A =:= 0) ) ->
	(	write('invalid username. Please try again.'),nl,nl,	user	);
	( (D =:= 1) ) ->
	(	write('password incorrect. Please try again.'),nl,nl,	user	)
).
notMatched(U,P,A,B,C,D,1):-
	write_to_file('username.txt',U),	write_to_file('password.txt',P),
	atomic_concat(U,'_D.txt',Za),	atomic_concat(U,'_T.txt',Z),	create_file(Z,''),	create_file(Za,''),
	write('New account created.'),nl,nl,	member_login(n).
logged_in(U):-
	write('Welcome '),write(U),nl,	explore(U).
explore(U):-
	history_check(U),	write('Do you want to explore more? (Type y if yes and n otherwise)'),nl,
	read(X),
(	(X='y')-> 
	(	explore(U));
	(X='n')-> 
...
logged_out(X).	
history_check(U):-
	atomic_concat(U,'_D.txt',Z),	atomic_concat(U,'_T.txt',Y),	read_history(Z,Y,0,0,0,U).
choice(T,D,U):-
(	((D=:=0),(T=:=0))->
	(	choose(U)	);
...
read_history(Z,Y,D,T,A,U):-
    open(Z, read, Str),    read_h_file(Str,Lines,Z,Y,D,T,A,U),    close(Str).
read_h_file(Stream,Lines,Z,Y,D,T,A,U):-
    \+ at_end_of_stream(Stream),    readLine(Stream,Lines),	ff(A,Lines),	B is 1,
    read_h_file(Stream,L,Z,Y,B,T,A,U).	read_h_file(Stream,Lines,Z,Y,D,T,A,U):-    at_end_of_stream(Stream),
...
history(1,Y,U):-
	atomic_concat(U,'_D.txt',Z),	check_from_userfile(Z,Y).
history(2,Y,U):-
	atomic_concat(U,'_T.txt',Z),	check_from_userfile(Z,Y).
check_from_userfile(File,U):-
    open(File, read, Str),    check_file(Str,Lines,File,U),    close(Str).
check_file(Stream,Lines,File,U):-
    \+ at_end_of_stream(Stream),    readLine(Stream,Lines),
	( (\+ (Lines = U))) ->
	(	check_file(Stream,L,File,U)).
check_file(Stream,Lines,File,U):-
    at_end_of_stream(Stream),write_to_file(File,U).
choose(U):-
write('		Which way you want to choose a place for your tour?     '), nl,
write('		1. Division Wise  '), nl,
write('		2. Tourism type wise   '), nl,
write('	Enter your choice : '),	read(C), choose_type(C,U).
...
write('	Enter your choice : '),nl,	read(Y), history(2,Y,U),	select_tour(Y,U).
select_tour(Y,U):-
Y==1, ecotourism;Y==2, study_tour; Y==3, adventure_tour; Y==4, natural_tour; Y==5, bicycle_tour; Y=\=1 , Y=\=2, Y=\=3, Y=\=4, Y=\=5,
...
write('	Enter your choice : '),nl,read(D), 		history(1,D,U), 	select_div(D,U).
...
write('	Enter your choice : '),nl,		read(P), select_dhaka(P).
...
write('			4. Brac		'), nl,
write('	Enter your choice : '),	read(St), select_study(St).
