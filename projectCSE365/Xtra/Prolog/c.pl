...
explore(U):-
	history_check(U),
	write('Do you want to explore more? (Type y if yes)'),nl,
	read(X),
	(X='y')-> 
	(
		explore(U)
...
read_h_file(Stream,Lines,Z,Y,D,T,A,U):-
	...
	read_h_file(Stream,Lines,Z,Y,D,T,A,U):-
    at_end_of_stream(Stream),
	(A=:=0)->	(read_history(Y,Z,T,D,1,U)); (main(D,T,U)).
...
check_from_userfile(File,U):-
    open(File, read, Str),
    check_file(Str,Lines,U),
    close(Str).
										
check_file(Stream,Lines,U):-
    at_end_of_stream(Stream),
	write_to_file(Z,Y).
